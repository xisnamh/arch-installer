...
menu_header
print_title "PRE-INSTALACION: SISTEMA"
...
if [ -d "/sys/firmware/efi" ]; then
	...
	if ! print_confirm "¿Deseas continuar de todos modos?"; then
		printf "${ico_error} ${redl}Instalacion de Arch Linux cancelada por el usuario. Saliendo del instalador...${end}\n"
		exit 1
	fi
fi
print_continue
